...
import com.java110.common.constant.ListenerDomain;
 ...
 * 
 */
public abstract class FtpToFileSystemQuartz extends LoggerEngine {


	protected IListenerServiceDao listenerServiceDaoImpl;
	...
	public void initTask() throws Exception{
		...
		int updateFtpItemRunStateFlag = listenerServiceDaoImpl.updateFtpItemRunState(paramIn);
	...
	public void callPreFunction(Map taskInfo) {
		...
		if (taskInfo.containsKey("PREFUNCTION") && taskInfo.get("PREFUNCTION") != null && !"".equals(taskInfo.get("PREFUNCTION"))) {
			try {
				getListenerServiceDao().saveDbFunction(taskInfo.get("PREFUNCTION").toString());
	...
	public void callAfterFunction(Map taskInfo) {
		...
		if (taskInfo.containsKey("AFTERFUNCTION") && taskInfo.get("AFTERFUNCTION") != null && !"".equals(taskInfo.get("AFTERFUNCTION"))) {
			try {
				...
				getListenerServiceDao().saveDbFunctionWithParam(taskInfo);
				taskInfo.put("threadrunstate", "T");
	 ...
	 * @return
	 */
	private void updateTaskState(long taskId, String state) throws Exception{
		Map info = new HashMap();

		info.put("taskId", taskId);
		info.put("runState", state);
		int updateFtpItemFlag = getListenerServiceDao().updateFtpItemByTaskId(info);
		// 这里只是后台提示，不进行日志保存
	...
	private void udpateTaskLog(Map taskInfo) {
		...
		getListenerServiceDao().updateTaskRunLog(loginfo);
	}
	...
	protected void saveTaskLogDetail(Map taskInfo) {
		...
		Long logdetailid = getListenerServiceDao().saveTaskRunDetailLog(logdetail);
	...
	private long insertTaskInfo(Map taskInfo) {
		...
		return getListenerServiceDao().saveTaskRunLog(loginfo);
	}
	...
	private void sendErrLogPhoneMsg(Map taskInfo, long taskLogID) {
		...
		if (phone != null && !"".equals(phone)) {
			...
			for (int i = 0; i < phonelist.length; i++) {
				...
				getListenerServiceDao().saveTaskErrInfoPhoneMsg(msginfo);
			}
		}*/
	}
	...
	protected List<String> dealFileName(String fileName) {
		...
		String result = "";
		// 文件中使用的日期
		if (StringUtils.contains(fileName, ListenerDomain.REPLAY_TYPE_F)) {
			result = StringUtil.replace(fileName, ListenerDomain.REPLAY_TYPE_F, DateUtil.getFormatTimeString(new Date(), "yyyyMMddHHmm"));
		} else if (StringUtils.contains(fileName, ListenerDomain.REPLAY_TYPE_E)) {
			result = StringUtil.replace(fileName, ListenerDomain.REPLAY_TYPE_E, DateUtil.getFormatTimeString(new Date(), "yyyyMMddHH"));
		} else if (StringUtils.contains(fileName, ListenerDomain.REPLAY_TYPE_A)) {
			result = StringUtil.replace(fileName, ListenerDomain.REPLAY_TYPE_A, DateUtil.getFormatTimeString(new Date(), "yyyyMMdd"));
		} else if (StringUtils.contains(fileName == null ? "" : fileName.toLowerCase(), ListenerDomain.REPLAY_TYPE_SQL)) {
			// 后期改造，文件名如果配置的是sql的话，以sql查询文件名
			List<String> fileNames = this.getListenerServiceDao().execConfigSql(fileName);
