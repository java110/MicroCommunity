 ...
 * 
 */
public class DownloadFileFromFtpToTable extends FtpToFileSystemQuartz {
	...
	private void anyFilesDownload(Map ftpItemConfigInfo, String taskId, FTPClientTemplate ftpClientTemplate, String ftpPath, List<String> fileNames) throws Exception {
		...
		for (String fileName : fileNames) {
			if (fileName.length() > 0) {
				...
				needDownloadFiles = this.listenerServiceDaoImpl.queryFileNamesWithOutFtpLog(logInfo);
			}
	...
	private void saveDownLoadSuccessFile(Map remoteFileInfo) throws Exception{
		// TODO Auto-generated method stub
		int addDownloadFlag = this.listenerServiceDaoImpl.addDownloadFileName(remoteFileInfo);
