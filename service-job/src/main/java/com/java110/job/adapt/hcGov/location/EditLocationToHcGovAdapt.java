 ...
 */
@Component(value = "editLocationToHcGovAdapt")
public class EditLocationToHcGovAdapt extends DatabusAdaptImpl {
    ...
    private void doAddCommunityLocation(Business business, JSONObject businessCommunityLocation) {
        ...
        communityLocationDto.setLocationId(communityLocationPo.getLocationId());
        communityLocationDto.setCommunityId(communityLocationPo.getCommunityId());
        ...
        Assert.listOnlyOne(communityLocationDtos, "位置不存在");
        ...
        communityLocationAttrDto.setLocationId(communityLocationDtos.get(0).getLocationId());
        communityLocationAttrDto.setSpecCd(HcGovConstant.EXT_COMMUNITY_ID);
        communityLocationAttrDto.setCommunityId(communityId);
        ...
        JSONObject kafkaData = null;
        if (communityLocationAttrDtos != null && communityLocationAttrDtos.size() > 0) {
            JSONObject body = new JSONObject();
            body.put("name", communityLocationDtos.get(0).getLocationName());
            body.put("extLocationId", communityLocationDtos.get(0).getLocationName());
            kafkaData = baseHcGovSendAsynImpl.createHeadersOrBody(body, extCommunityId, HcGovConstant.EDIT_LOCATION_ACTION, HcGovConstant.COMMUNITY_SECURE);
        } else {
            JSONObject body = new JSONObject();
            body.put("name", communityLocationDtos.get(0).getLocationName());
            kafkaData = baseHcGovSendAsynImpl.createHeadersOrBody(body, extCommunityId, HcGovConstant.ADD_LOCATION_ACTION, HcGovConstant.COMMUNITY_SECURE);
        }
