...
public class CommunityApi extends BaseController {
    private final static Logger logger = LoggerFactory.getLogger(CommunityApi.class);

    @Autowired
    ICommunityServiceSMO communityServiceSMOImpl;

    @RequestMapping(path = "/communityApi/service",method= RequestMethod.GET)
     ...
     * @return
     */
    @RequestMapping(path = "/communityApi/service",method= RequestMethod.POST)
    public String servicePost(@RequestBody String orderInfo, HttpServletRequest request) {
        ...
        try {
            ...
            responseJson = communityServiceSMOImpl.service(businessServiceDataFlow);
