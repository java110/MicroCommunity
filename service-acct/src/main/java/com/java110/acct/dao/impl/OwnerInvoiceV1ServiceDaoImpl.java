...
public class OwnerInvoiceV1ServiceDaoImpl extends BaseServiceDao implements IOwnerInvoiceV1ServiceDao {
     ...
     */
    @Override
    public int updateOwnerInvoiceInfo(Map info) throws DAOException {
        logger.debug("修改 updateOwnerInvoiceInfo 入参 info : {}",info);

        int saveFlag = sqlSessionTemplate.update("ownerInvoiceV1ServiceDaoImpl.updateOwnerInvoiceInfo",info);
     ...
     */
    @Override
    public int queryOwnerInvoicesCount(Map info) {
        logger.debug("查询 queryOwnerInvoicesCount 入参 info : {}",info);

        List<Map> businessOwnerInvoiceInfos = sqlSessionTemplate.selectList("ownerInvoiceV1ServiceDaoImpl.queryOwnerInvoicesCount", info);
        if (businessOwnerInvoiceInfos.size() < 1) {
            return 0;
        }

        return Integer.parseInt(businessOwnerInvoiceInfos.get(0).get("count").toString());
    }
