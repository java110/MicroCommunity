...
public class InvoiceApplyItemV1ServiceDaoImpl extends BaseServiceDao implements IInvoiceApplyItemV1ServiceDao {
     ...
     */
    @Override
    public int queryInvoiceApplyItemsCount(Map info) {
        logger.debug("查询 queryInvoiceApplyItemsCount 入参 info : {}",info);

        List<Map> businessInvoiceApplyItemInfos = sqlSessionTemplate.selectList("invoiceApplyItemV1ServiceDaoImpl.queryInvoiceApplyItemsCount", info);
        if (businessInvoiceApplyItemInfos.size() < 1) {
            return 0;
        }

        return Integer.parseInt(businessInvoiceApplyItemInfos.get(0).get("count").toString());
    }
