 ...
 */
@Java110Listener("deleteMsgReadInfoListener")
@Transactional
public class DeleteMsgReadInfoListener extends AbstractMsgReadBusinessServiceDataFlowListener {

    private final static Logger logger = LoggerFactory.getLogger(DeleteMsgReadInfoListener.class);
    @Autowired
    IMsgReadServiceDao msgReadServiceDaoImpl;
    ...
    public String getBusinessTypeCd() {
        return BusinessTypeConstant.BUSINESS_TYPE_DELETE_MSG_READ;
    }
    ...
    protected void doBusinessToInstance(DataFlowContext dataFlowContext, Business business) {
        ...
        if (businessMsgReadInfos != null && businessMsgReadInfos.size() > 0) {
            for (int _msgReadIndex = 0; _msgReadIndex < businessMsgReadInfos.size(); _msgReadIndex++) {
                ...
                flushBusinessMsgReadInfo(businessMsgReadInfo, StatusConstant.STATUS_CD_INVALID);
