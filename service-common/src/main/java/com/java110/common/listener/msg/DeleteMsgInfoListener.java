 ...
 */
@Java110Listener("deleteMsgInfoListener")
@Transactional
public class DeleteMsgInfoListener extends AbstractMsgBusinessServiceDataFlowListener {

    private final static Logger logger = LoggerFactory.getLogger(DeleteMsgInfoListener.class);
    @Autowired
    IMsgServiceDao msgServiceDaoImpl;
    ...
    public String getBusinessTypeCd() {
        return BusinessTypeConstant.BUSINESS_TYPE_DELETE_MSG;
    }
    ...
    protected void doBusinessToInstance(DataFlowContext dataFlowContext, AppBusiness business) {
        ...
        if (businessMsgInfos != null && businessMsgInfos.size() > 0) {
            for (int _msgIndex = 0; _msgIndex < businessMsgInfos.size(); _msgIndex++) {
                ...
                flushBusinessMsgInfo(businessMsgInfo, StatusConstant.STATUS_CD_INVALID);
     ...
     */
    private void doBusinessMsg(AppBusiness business, JSONObject businessMsg) {
